...
- Pre-existing red state is not chased as part of this work: the `taplo fmt --check` failures in `make lint-extra`. Note that this workspace is otherwise green — measured at `001314f`, `cargo nextest run --workspace --all-features` gives 2744 passed, 6 skipped. The failing-tests-at-`main` note in this project's history belongs to the sibling `../praxis` proxy repo, not here.
...
- **Enforcement surface:** clippy, rustdoc, MSRV, and release packaging all run default features today, so their coverage becomes feature-dependent the moment the merged crate is `default = []`. U1 restores them. This is the plan's most easily-missed regression because nothing fails — the gates simply go quiet.
  ...
  The scale is larger than a feature-gating argument suggests. Today each builtin is its own workspace member, so all nine are tested directly in **both** passes: measured at `001314f`, they contribute 661 tests (JWT 185, OAuth 115, API key 81, OPA 77, Cedar 62, CEL 41, Vault 40, CIBA 39, Valkey 21) to a 2744-test all-features run and to the 2712-test default run alike. Afterwards the merged crate is one member at `default = []`, and a default `--workspace` pass enables only what `ppe-pdp-diff` requests — cedar, cel, opa — so pass 1 carries roughly 180 builtin tests instead of 661. The other ~481 are reachable only under `--all-features`, which is why the U1 gate work is load-bearing rather than tidy-up.
...
| `release.yaml`'s `verify` job gates on `make lint` and `make test`, so any regression there blocks the release outright | Low | High | Measured green at U1 (2744 passed, 6 skipped), so no pre-existing blocker exists. The residual risk is a regression introduced by this work, which the per-unit gates catch |

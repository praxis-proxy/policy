...
| `identity/api-key` | identity | `api-key` | Resolve a subject by looking an opaque API key up in a directory, from a file or over HTTP. See [Recipe 7](identity-delegation.md#recipe-7-an-opaque-api-key-resolved-against-a-directory). |

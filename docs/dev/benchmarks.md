...
p95/p99 by default. Benches use `SamplingMode::Flat` so each sample runs the
same iteration count; `bench_percentiles.py` divides `sample_time / iters` and
takes p50/p95/p99 across those sample means. After a bench run:
...
make bench-heap
# → prints per_decision_bytes and max_bytes vs policy count; writes
...
| What *does* CI do? | `make lint` runs workspace `clippy --all-targets` plus `clippy -p ppe-benches --features dhat-heap` so every `[[bench]]` (including `heap_profile`) compiles. `make coverage` excludes `ppe-benches` from the floor. |
...
(`Flat` sampling: `time_per_iter = sample_time / iters` per sample; percentiles
are across sample means). Means included for comparison. Re-run `make bench`
before refreshing these numbers.

...
fn memory(c: &mut Criterion) {
    ...
    for &n_labels in &[8_usize, 64, 512] {
        ...
        // stable. Return the store so Criterion drops it after timing (not
        // inside the measured window). This path still uses `block_on` per
        // iter; compare shape vs `session_load_labels`, not absolute ns.

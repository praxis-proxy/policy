...
//! `PdpResolver::evaluate`. All three fixtures enforce the same rule:
//! principal `alice` with the `reader` role may read.
...
fn pdp_cost(c: &mut Criterion) {
    ...
    let cel_args = cel_call("has(role.reader) && role.reader");
    {
        let d = rt
    ...
    const OPA_MODULE: &str = "package authz\n\
default allow := false\n\
allow if input.role.reader == true\n";

...
pub async fn evaluate_pre(
    ...
    for rule in &route.args {
        ...
        // redacts the `ssn` of every element of `rows`, not nothing. A plain
        // object path expands to itself, so non-array fields are unchanged.
...
pub async fn evaluate_post(
    ...
    if let Some(result) = payload.result.as_mut() {
        for rule in &route.result {
            // Fan out over arrays on the path (e.g. redact `rows.ssn` on every
            // row); an object-only path expands to itself. An over-large shape
            // fails closed rather than half-redacting.
...
/// Index one JSON value by a single path segment. An object is indexed by the
/// segment as a key; an array is indexed by the segment parsed as a base-10
/// index. Anything else (or an out-of-range / non-numeric array index) is
/// `None`. Shared by the read/write/remove helpers so a numeric segment means
...
/// along the way are accounted for. A field pipeline (`result.rows.ssn |
/// redact`) targets a leaf; when an *intermediate* segment is an array, the
/// leaf exists once per element, so redaction must reach every element — not
/// silently no-op because `Value::get("rows")` on... an array of objects
/// returns the object but the *next* hop lands on an array. Each array
/// encountered before the final segment fans out into one concrete path per
/// index (`rows.0.ssn`, `rows.1.ssn`, …). Paths whose leaf is absent on a
/// given element are dropped (consistent with the missing-field skip rule).
///
/// A terminal array is left as a single path (`result.rows | redact` replaces
/// the whole array), preserving existing whole-value pipeline semantics.
///
/// Returns `None` when the expansion would exceed a fan-out bound (array
/// nesting deeper than [`MAX_FANOUT_DEPTH`], or more than [`MAX_EXPANDED_PATHS`]
/// leaves). Fanning out over attacker-shaped tool output (`payload.result`) is
/// otherwise unbounded in stack depth and allocation, so an over-large shape
/// fails closed: the caller denies rather than either recursing without limit
/// or half-redacting and passing the tail through unredacted.
pub(crate) fn expand_field_paths(root: &serde_json::Value, path: &str) -> Option<Vec<String>> {
    ...
    fn walk(
        ...
        // All segments consumed — `prefix` is a concrete path to a leaf.
        ...
        let Some(child) = segment_get(value, seg) else {
            return true; // missing segment on this branch → skip (no path emitted)
        };
...
/// failing closed. High enough for legitimate bulk results, low enough to bound
/// the work and allocation an adversarial result shape can force.
...
/// through arrays the same way `get_dotted` reads through them (a path from
/// [`expand_field_paths`] carries numeric segments for array elements).
fn parent_mut<'a>(
...
mod tests {
    ...
    fn expand_field_paths_fans_out_over_intermediate_arrays() {
        ...
        // A terminal array is one path — whole-value pipeline semantics.
        assert_eq!(
    ...
    fn expand_field_paths_fails_closed_on_deep_nesting() {
        // A pathologically deep array nesting must fail closed (None), not
        // recurse without bound. Build a value nested past MAX_FANOUT_DEPTH.
    ...
    async fn result_pipeline_redacts_every_array_element() {
        // The finding: `result.rows.ssn | redact` must reach every row, not
        // silently no-op because `rows` is an array. Without the fan-out the
        // SSNs would pass through unredacted — a PII fail-open.
        let mut route = CompiledRoute::new("test");
        route.result.push(field_rule(
            "rows.ssn",
            vec![Stage::Redact { condition: None }],

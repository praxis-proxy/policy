...
Criterion does not print p95/p99 by default. The benches use
``SamplingMode::Flat``, so every sample runs the same iteration count and
``sample_time_ns / iters`` is a per-iteration mean for that sample.
Percentiles are taken across those sample means (not across individual
iterations inside a sample).

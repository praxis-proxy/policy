...
impl OAuthDelegator {
    ...
    /// code.
    async fn exchange(&self, payload: &DelegationPayload) -> Result<Mint, PluginViolation> {
...
impl HookHandler<TokenDelegateHook> for OAuthDelegator {
    async fn handle(
        ...
        let served = match self.cache_for(payload) {
            Some((cache, key)) => match cache.get_or_mint(key, self.exchange(payload)).await {
                Ok(served) => served,
            ...
            None => match self.exchange(payload).await {
                Ok(mint) => Served {
                    mint,
                    source: Source::Mint,
                    minted_at: Utc::now(),
